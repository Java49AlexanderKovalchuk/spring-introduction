...
record PersonIdString (String id, String  name, String city, String email, String phone) {
...
public class GreetingsControllerTest {
    ...
    Person personWrongName = new Person(124, "john", "Rehovot", "vasya@gmail.com",
    		"054-1234567");
    Person personWrongCity = new Person(124, "John", null, "vasya@gmail.com",
    		"054-1234567");
    Person personWrongEmail = new Person(123, "Vasya", "Rehovot", "vasyagmail.com",
    		"054-1234567");
    PersonIdString personWrongId = 
    		new PersonIdString ("abs", "Vasya", "Rehovot", "vasya@gmail.com", "054-1234567");
    ...
    ObjectMapper objectMapper;
     ...
     void addPersonWrongId() throws Exception {
    	 ...
    	 assertEquals("JSON parse error: Cannot deserialize value of type `long`"
    	 		+ " from String \"abs\": not a valid `long` value",	response);
     }
     //***
     @Test
     void updatePersonTest() throws Exception {
    	 when(greetingsService.updatePerson(personNormal)).thenReturn(personNormal);
    	 String personJson = objectMapper.writeValueAsString(personNormal);
     ...
     void updatePersonNotFoundTest() throws Exception {
    	 when(greetingsService.updatePerson(personNormalUpdated))
    	 	.thenThrow(new NotFoundExeption(PERSON_NOT_FOUND));
     ...
     void getPersonTest() throws Exception {
    	 long id = 123;
    	 long idNotExist = 124;
    	 when(greetingsService.getPerson(123)).thenReturn(personNormal);
    	 when(greetingsService.getPerson(124)).thenReturn(null);
    	 String personJson = objectMapper.writeValueAsString(personNormal);
     ...
     void getPersonByCityTest() throws Exception {
    	 String city = "Rehovot";
    	 when(greetingsService.getPersonByCity(city)).thenReturn(List.of(personNormal, personNormalUpdated));
    	 Person[] expected = {
    			 personNormal, 
    			 personNormalUpdated
    	 		};
     ...
     void deletePersonNotFoundTest() throws Exception {
    	 long id = 124;
    	 when(greetingsService.deletePerson(id)).thenThrow(new NotFoundExeption(PERSON_NOT_FOUND));
    	 String response = mockMvc.perform(delete("http://localhost:8080/greetings/" + id))
    	    	 .andDo(print()).andExpect(status().isNotFound())
    	    	 .andReturn().getResponse().getContentAsString();
    	 assertEquals(PERSON_NOT_FOUND, response);
